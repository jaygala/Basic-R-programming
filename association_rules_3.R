...
names(itemlist) <- paste("Tr",c(1:9), sep = "")   #just giving a name to every list
itemlist
## coerce into transactions
tdata3 <- as(itemlist, "transactions")    #convert to transaction format i.e. like that groceries wala format. 
tdata3
summary(tdata3)
...
#Analysis
freqitems = eclat(tdata)   #shows the ccount of each subset of the items..   #default support=.1    # this shows how each item was bought. this set is larger than the number of transactions (obviously)
